...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkCommunicationNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkCommunicationNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkCommunicationNetworkObject.Length > 0 && NetworkCommunicationNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkCommunicationNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkCommunicationBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkCommunication instead, its shorter and easier to type out ;)")]
		public NetworkCommunicationBehavior InstantiateNetworkCommunicationNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkCommunicationNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkCommunicationBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkCommunicationBehavior>().networkObject = (NetworkCommunicationNetworkObject)obj;
		...
		public NetworkCommunicationBehavior InstantiateNetworkCommunication(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkCommunicationNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkCommunicationBehavior>();
